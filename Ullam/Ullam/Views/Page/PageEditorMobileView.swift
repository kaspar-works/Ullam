...
struct PageEditorMobileView: View {
    ...
    @State private var suggestedMoodEmoji: String?
    @State private var suggestedMoodLabel: String?
    @State private var showMoodSuggestion = false
    @State private var moodDetectionTimer: Timer?
    ...
    // Auto-destruct state
    @State private var showAutoDestructSheet = false
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        // Tag chips
                        if !page.tagIds.isEmpty && !isFocusMode {
                            TagChipsView(tagIds: page.tagIds, diary: page.diary) {
                        ...
                        // AI mood suggestion pill
                        if showMoodSuggestion, let emoji = suggestedMoodEmoji, let label = suggestedMoodLabel, !isFocusMode {
                            moodSuggestionPill(emoji: emoji, label: label)
                                .padding(.horizontal, 24)
                                .padding(.top, 10)
                ...
                if showToolbar {
                    VStack(spacing: 6) {
                        // Ambiance floating control
                        HStack {
                            Spacer()
                            AmbianceControlView(ambianceService: ambianceService)
                                .padding(.trailing, 14)
                        }
                        ...
                        FloatingToolbar(
                            ...
                            onSketchTap: { showingSketchSheet = true },
        ...
        .sheet(isPresented: $showingTagPicker) {
            if let diary = page.diary {
                TagPickerView(page: page, diary: diary)
                    .presentationDetents([.medium])
                    .presentationBackground(.ultraThinMaterial)
            }
        }
        .sheet(isPresented: $showingSketchSheet) {
            SketchSheetView(onSave: { pngData in
                withAnimation(.spring(response: 0.3)) {
                    attachedImageData.append(pngData)
                }
                saveNow()
            })
        ...
        .sheet(isPresented: $showAutoDestructSheet) {
            AutoDestructSheetView(page: page) {
                showAutoDestructSheet = false
            }
            .presentationDetents([.medium])
        ...
        HStack(spacing: 10) {
            ...
            if !isFocusMode {
                ...
                Button {
                    ...
                    Image(systemName: isBookmarked ? "bookmark.fill" : "bookmark")
                        ...
                        .foregroundStyle(isBookmarked ? AppTheme.accent.opacity(0.8) : AppTheme.dimText)
                ...
                Button {
                    showAutoDestructSheet = true
                    UIImpactFeedbackGenerator(style: .light).impactOccurred()
                } label: {
                    Image(systemName: page.autoDestructDate != nil ? "timer" : "timer")
                        .font(.system(size: 13, weight: .medium))
                        .foregroundStyle(page.autoDestructDate != nil ? Color(hex: 0xF87171).opacity(0.8) : AppTheme.dimText)
                        .frame(width: 34, height: 34)
                        .background(page.autoDestructDate != nil ? Color(hex: 0xF87171).opacity(0.08) : AppTheme.subtle)
                        .clipShape(Circle())
                        .overlay(Circle().stroke(page.autoDestructDate != nil ? Color(hex: 0xF87171).opacity(0.15) : AppTheme.subtle, lineWidth: 1))
                }
                .buttonStyle(ToolbarButtonStyle())
            ...
            Button {
                ...
                Text("Done")
                    ...
                    .background(
                        Capsule()
                            .fill(LinearGradient(colors: [AppTheme.accent.opacity(0.5), Color(hex: 0xC49340).opacity(0.4)], startPoint: .leading, endPoint: .trailing))
            ...
            ForEach(Array(selectedEmojis.enumerated()), id: \.offset) { index, emoji in
                Button {
                    ...
                    Text(emoji)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 8, style: .continuous).stroke(AppTheme.subtle, lineWidth: 1))
                }
                .buttonStyle(ToolbarButtonStyle())
    ...
    private var separatorLine: some View {
        LinearGradient(colors: [.clear, AppTheme.subtle, .clear], startPoint: .leading, endPoint: .trailing)
            .frame(height: 1)
        ...
        VStack {
            HStack {
                ...
                Button {
                    ...
                    .background(.ultraThinMaterial.opacity(0.6))
                    .clipShape(Capsule())
                    .overlay(Capsule().stroke(AppTheme.subtle, lineWidth: 1))
                }
                .buttonStyle(ToolbarButtonStyle())
    ...
    // MARK: - Mood Suggestion Pill

    private func moodSuggestionPill(emoji: String, label: String) -> some View {
        Button {
            Task {
                await diaryManager.setDayMood(emoji, for: date)
                withAnimation(.spring(response: 0.3)) {
    ...
    // MARK: - Mood Detection Trigger

    private func triggerMoodDetection() {
        moodDetectionTimer?.invalidate()
        moodDetectionTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: false) { _ in
            Task { @MainActor in
                // Only suggest if no mood is set for today
                guard diaryManager.getDayMood(for: date) == nil else { return }
                guard !showMoodSuggestion else { return }

                let text = attributedContent.string
                guard text.count >= 20 else { return }

                if let result = MoodDetectionService.shared.detectMood(from: text) {
                    suggestedMoodEmoji = result.emoji
                    suggestedMoodLabel = result.label
    ...
    private func markTyping() {
        ...
        // Trigger mood detection after typing pause
