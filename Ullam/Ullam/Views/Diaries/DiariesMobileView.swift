...
struct DiariesMobileView: View {
    ...
    @State private var readyCapsuleCount = 0
    @State private var showTimeCapsules = false
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if let current = diaryManager.currentDiary {
                        heroDiaryCard(current)
                            ...
                            .offset(y: appeared ? (heroFloat ? -3 : 3) : 14)
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 6) {
                ...
                Text(greetingEmoji)
                    ...
                    .animation(.spring(response: 0.4, dampingFraction: 0.5).delay(0.3), value: appeared)
            }
            ...
            ZStack {
                ...
                .background(
                    Capsule()
                        ...
                        .overlay(
                            Capsule()
                                .stroke(AppTheme.mutedText.opacity(0.18), lineWidth: 1)
                        )
                )
            }
    ...
    // MARK: - Time Capsule Card

    private var timeCapsuleCard: some View {
        NavigationLink(destination: TimeCapsuleListView(diaryManager: diaryManager)) {
            HStack(spacing: 12) {
                ZStack {
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 10, style: .continuous)
                                .stroke(AppTheme.moodHappy.opacity(0.12), lineWidth: 1)
                        )
                    Image(systemName: "shippingbox.fill")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundStyle(AppTheme.moodHappy.opacity(0.6))
                }
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        ...
                        if readyCapsuleCount > 0 {
                            Text("\(readyCapsuleCount)")
                                .font(.system(size: 10, weight: .bold))
                                .foregroundStyle(AppTheme.primaryText)
                                .frame(width: 18, height: 18)
                                .background(
                                    Circle()
                                        .fill(AppTheme.moodHappy)
                                )
                        }
                    }
                ...
                Image(systemName: "chevron.right")
                    .font(.system(size: 11, weight: .semibold))
                    .foregroundStyle(AppTheme.moodHappy.opacity(0.35))
            }
            .padding(14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .stroke(AppTheme.moodHappy.opacity(0.08), lineWidth: 1)
                    )
            )
        }
        .buttonStyle(SanctuaryButtonStyle())
    ...
    private func refreshAll() {
        ...
        // Load ready-to-open capsule count
        if let diary = diaryManager.currentDiary {
            let context = DataController.shared.container.mainContext
            readyCapsuleCount = TimeCapsuleService.shared.getUnlockedCapsules(diary: diary, context: context).count
        }
    }
