            ...
            if isSearching {
                ...
                ScrollView {
                    LazyVStack(spacing: 8) {
                        ForEach(Array(results.enumerated()), id: \.element.id) { idx, page in
                            SearchResultRow(page: page, diaryManager: diaryManager, dateFormatter: dateFormatter)
                                .onTapGesture {
                                    dismiss()
                                }
                                .transition(.asymmetric(
                                    insertion: .opacity.combined(with: .offset(y: 8)).combined(with: .scale(scale: 0.97)),
                                    removal: .opacity
                                ))
                                .animation(.spring(response: 0.4, dampingFraction: 0.8).delay(Double(idx) * 0.04), value: results.count)
                        }
                    }
                    .padding(.horizontal, 20)
                    .padding(.top, 12)
