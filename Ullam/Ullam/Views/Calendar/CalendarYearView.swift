...
struct CalendarYearView: View {
    ...
    @State private var recentExcerpt: String? = nil
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: 24) {
                    ...
                    filterPill(color: AppTheme.secondaryText, label: "Reflective")
                }
                .padding(.horizontal, 24)
                .padding(.bottom, 20)
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(
                                selectedDay == day ? AppTheme.accent.opacity(0.4) : AppTheme.subtle,
                                lineWidth: 1
                            )
                    )
            )
    ...
    private var streakSection: some View {
        HStack(spacing: 24) {
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                // Excerpt overlay
                if let excerpt = recentExcerpt, !excerpt.isEmpty {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                Text(currentStreak > 0
                     ? "\(currentStreak) \(currentStreak == 1 ? "Night" : "Nights") of\nConsecutive Reflection"
                     : "No Current\nStreak")
                ...
                // Stat cards
                HStack(spacing: 12) {
                    statCard(value: formattedWordCount(totalWords), label: "WORDS PENNED")
                }
    ...
    private func loadYearMoods() async {
        ...
        // Compute streak from all pages
        let streakData = StreakTracker.calculateStreak(pages: allPages)
        currentStreak = streakData.currentStreak

        // Compute total words for the selected month
        ...
        let monthPages = diaryManager.getPages(from: monthStart, to: monthEnd)
        var words = 0
        ...
        // Extract a recent excerpt for the quote area
        if let latestPage = monthPages.first {
            if let content = latestPage.plaintextContent {
                let text = (try? NSAttributedString(
                    data: content,
                    options: [.documentType: NSAttributedString.DocumentType.rtf],
                    documentAttributes: nil
                ...
                if !trimmed.isEmpty {
                    recentExcerpt = String(trimmed.prefix(120))
                } else {
                    recentExcerpt = nil
                }
            } else {
