...
struct AIWritingSheet: View {
    ...
    enum AIWritingAction: String, CaseIterable, Identifiable {
        ...
        var gradient: [Color] {
            switch self {
            case .improve: return [Color(hex: 0xD4A24C), Color(hex: 0xC49340)]
            case .summarize: return [Color(hex: 0x5B8A72), Color(hex: 0x4A7560)]
            case .rewrite: return [Color(hex: 0xC07070), Color(hex: 0xA85858)]
            case .expand: return [Color(hex: 0x4A5A7A), Color(hex: 0x3A4A68)]
            case .fixGrammar: return [Color(hex: 0x8890A0), Color(hex: 0x6B7080)]
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Handle
            Capsule()
                .fill(AppTheme.mutedText.opacity(0.25))
    ...
    private func accessoryButton(icon: String, label: String?, action: @escaping () -> Void) -> some View {
        Button {
            ...
            .foregroundStyle(icon == "sparkles" ?
                ...
                AnyShapeStyle(AppTheme.mutedText)
            )
